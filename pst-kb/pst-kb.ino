...
void loop() {
  if (keyboard.available()) {
    ...
    if (c > 0) {
      ...
      bool isBreakCode = c & 0x8000;

      if (!isBreakCode) {

        uint8_t keyCode = c & 0xFF;      // Extract the key code
                                         // if (keyCode >= 32 && keyCode <= 126) {  // Printable ASCII range
        char character = (char)keyCode;  // Convert keyCode to ASCII character
        // Serial.print("Character: ");
        Serial.print(character);
        /* } else {
        Serial.println("Non-printable character or special key");
      } */
      }
    }
  }
